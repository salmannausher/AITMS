...
import { createTruckSchema } from '@aitms/shared';
import { z } from 'zod';
...
export function OnboardingStep3({ onNext, onBack }: Props) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const { register, handleSubmit, setValue, formState: { errors } } = useForm<Step3Values>({
    resolver: zodResolver(step3Schema),
    defaultValues: { cdl_class: 'A', truck_type: 'DRY_VAN' },
  });
  ...
  async function onSubmit(data: Step3Values) {
    ...
    setError(null);
    try {
      // 1. Create truck first (no driver yet)
      ...
      // 2. Create driver, assigned to the truck
        ...
        <Section title="Driver">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Field label="CDL Class *" error={errors.cdl_class?.message}>
              <Select defaultValue="A" onValueChange={(v) => setValue('cdl_class', v as 'A' | 'B' | 'C', { shouldValidate: true })}>
            ...
            <Field label="Home State *" error={errors.home_state?.message}>
              <Select onValueChange={(v) => setValue('home_state', v, { shouldValidate: true })}>
        ...
        <Section title="Truck">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Truck Type *" error={errors.truck_type?.message}>
              <Select defaultValue="DRY_VAN" onValueChange={(v) => setValue('truck_type', v as Step3Values['truck_type'], { shouldValidate: true })}>
                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {TRUCK_TYPES.map((t) => (
